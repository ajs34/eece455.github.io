...
class des {
  ...
  applyTable(input, tableName) {
    try {
      if (tableName == "ip") return this.permute(input, this.ip);
      else if (tableName == "e") return this.permute(input, this.e);
      else if (tableName == "pc2") return this.permute(input, this.pc_2);
      else if (tableName == "pc1") return this.permute(input, this.pc_1);
      else if (tableName == "invip") return this.permute(input, this.inv_ip);
      else if (tableName == "p") return this.permute(input, this.p);
    } catch (error) {}
  }
  performSubBox(input) {
  ...
  encrypt(inverse = false) {
    if (inverse) this.steps += `Performing Decryption:\n`;
    else this.steps += `Performing Encryption:\n`;
